...
const NAV_GROUPS: Array<{
  label: string;
  items: Array<{ id: SettingsSection; label: string; icon: React.ComponentType<{ size?: number; weight?: any }> }>;
}> = [
  {
    label: 'Paramètres',
    items: [
...
export default function DesktopSettingsModal() {
  ...
  const codeSyntaxHighlight = useSettingsStore((state) => state.codeSyntaxHighlight);
  ...
  const toggleStream = useSettingsStore((state) => state.toggleStream);
  const toggleSyntax = useSettingsStore((state) => state.toggleSyntax);

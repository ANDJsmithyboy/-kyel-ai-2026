/* Nkyel AI · Nkyel AI.ts · SmartANDJ AI Technologies · Constitution Zion Core
...
export const PLANS: Record<string, PlanConfig> = {
  flash: { name: 'Nkyel Flash', price: 0, currency: 'FCFA', tokens: 50, tokensReset: 'daily', color: '#22C55E' },
  pro: { name: 'Nkyel Pro', price: 2500, currency: 'FCFA', tokens: 500, tokensReset: 'one-time', color: '#38BDF8' },
  elite: { name: 'Nkyel Élite', price: 13000, currency: 'FCFA', tokens: 3000, tokensReset: 'one-time', color: '#FACC15' },
};

/* -- State ------------------------------------------ */
interface NkyelState {
  plan: PlanKey;
  tokens: number;
  tokensMax: number;
...
const STORAGE_KEY = 'Nkyel AI_state';

function loadPersisted(): Partial<NkyelState> {
...
function persist(state: Partial<NkyelState>) {
  if (typeof window === 'undefined') return;
  try {
    const { isPaymentModalOpen, isUpgradeModalOpen, paymentStep, targetPlan, targetOperator, isThinking, thinkingMessage, ...rest } = state as NkyelState;
    localStorage.setItem(STORAGE_KEY, JSON.stringify(rest));
  } catch { /* noop */ }
}

const defaults: Omit<NkyelState, 'consumeTokens' | 'creditTokens' | 'setPlan' | 'openPaymentModal' | 'closePaymentModal' | 'openUpgradeModal' | 'closeUpgradeModal' | 'setThinking' | 'setUser' | 'reset'> = {
  plan: 'flash',
  tokens: 50,
  tokensMax: 50,
  isPantherMode: false,
...
export const useNkyelStore = create<NkyelState>((set, get) => ({
  ...
  reset: () => {
    set(defaults);
    persist(defaults as NkyelState);
    if (typeof window !== 'undefined') document.body.classList.remove('mode-bp', 'panther');
  },
}));

/* -- Derived selectors -------------------------------- */
export const selectTokensPercent = (s: NkyelState) =>

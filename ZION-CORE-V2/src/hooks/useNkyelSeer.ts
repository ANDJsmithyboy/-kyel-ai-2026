...
interface UseNkyelVisionOptions {
  fps?: number;
  enabled?: boolean;
  apiUrl?: string;
}

interface UseNkyelVisionReturn {
  isActive: boolean;
  description: string;
  error: string | null;
  toggle: () => void;
...
export function useNkyelVision({
  fps = 1,
  enabled = true,
  apiUrl = '/api/vision/analyze',
}: UseNkyelVisionOptions = {}): UseNkyelVisionReturn {

...
class GabomaSeerClient:
    """
    Client pour communiquer avec GabomaSeer (Oryx-1.5-32B) hébergé sur RunPod.
    """
    def __init__(self, base_url: Optional[str] = None, api_key: Optional[str] = None):
        # On utilise la variable d'environnement ou l'URL fournie
        self.base_url = base_url or os.getenv("GABOMASEER_URL", "http://localhost:8080")
        self.api_key = api_key or os.getenv("GABOMASEER_API_KEY", "")
    ...
    async def analyze_media(self, files: List[UploadFile], prompt: str) -> str:
        ...
        try:
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                # Appel au serveur RunPod / GabomaSeer
            ...
            print(f"Erreur de connexion à GabomaSeer ({exc}). Utilisation du mock.")
            return f"[GABOMASEER MOCK] Je vois les médias que tu m'as envoyés, mais je ne suis pas connecté au RunPod. Prompt reçu: '{prompt}'."
        except Exception as e:
            return f"Erreur lors de l'analyse GabomaSeer: {str(e)}"

# Instance singleton par défaut
gabomaseer_client = GabomaSeerClient()

"""
GabomaAI · Router : Vision
POST /api/vision/analyze — proxy vers GabomaSeer.
"""
...
async def analyze_vision(
    ...
    """
    Analyse visuelle via GabomaSeer.
    Accepte soit des fichiers uploadés, soit une image en base64 (mode Live).
    """
    result_text = ""

    # Mode 1 : Base64 (Live GabomaSeer — frames caméra)
    if image_b64:
        result_text = await gabomaseer.analyze_image_b64(image_b64, prompt)

    # Mode 2 : Fichiers uploadés
    elif files:
        result_text = await gabomaseer.analyze_files(files, prompt)

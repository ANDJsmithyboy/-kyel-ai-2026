"""
Ñkyel AI · NkyelSeer Client
Wrapper pour HF Inference API (Oryx-1.5-32B) ou instance self-hosted.
Sous-système vision de l'écosystème Ñkyel AI.
"""
...
class NkyelSeerClient:
    """Client vision NkyelSeer — appelle HF Inference API ou self-hosted."""

    def __init__(self):
        self.mode = settings.nkyelseer_mode
        self.hf_key = settings.hf_api_key
        self.self_hosted_url = settings.nkyelseer_url
        ...
        try:
            async with httpx.AsyncClient(timeout=60.0) as client:
                ...
                if response.status_code == 503:
                    ...
                    return (
                        f"NkyelSeer est en cours de chargement sur le serveur. "
                        f"Temps estimé : {wait_time:.0f}s. Réessayez dans un instant."
                    )

                if response.status_code == 429:
                    return "NkyelSeer : limite de requêtes atteinte. Réessayez dans quelques secondes."
            ...
            return "NkyelSeer : délai d'attente dépassé. Le modèle est peut-être en cours de chargement."
        except httpx.HTTPStatusError as e:
            return f"NkyelSeer : erreur HTTP {e.response.status_code}."
        except Exception as e:
            return f"NkyelSeer : erreur inattendue — {str(e)}"
    ...
    async def _call_self_hosted(self, url: str, headers: dict, image_b64: str, prompt: str) -> str:
        ...
        try:
            ...
            return f"NkyelSeer (self-hosted) : erreur — {str(e)}"

    def _mock_response(self, prompt: str) -> str:
        return (
            f"[NkyelSeer · Mode développement]\n"
            ...
            f"Dans un environnement de production, NkyelSeer (Oryx-1.5-32B) "
            f"fournirait ici une description détaillée de l'image."
        )


# Singleton
nkyelseer = NkyelSeerClient()
